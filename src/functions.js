...
module.exports = {
  ...
  getMedia: async function (account) {
    if (account.name == "CablePorn") {
      ...
      return console.log(
        `Database: Error, Cannot find account ${account.name}`
      );
    }
  },
  getPosts: async function (account) {
    axios
      .get(
        `https://www.reddit.com/r/${account.name.toLowerCase()}.json?sort=top&t=week&limit=800`
      )
      .then((res) => {
        const posts = res.data.data.children;
        posts.forEach(async (reddit) => {
          if (reddit.data.url_overridden_by_dest == null)
            return console.log("no media");
          ...
          db.findOne({ PostId: data.id }, async (err, post) => {
            if (!post) {
              ...
              const path = Path.join(
                __dirname,
                `../images/${account.name}/${data.id}.jpg`
              );
              await download(url, path, () => {
                console.log(
                  `${account.name.toUpperCase()}: 🙆‍♀️ ${
                    data.id
                  } has been downloaded.`
                );
              });
  ...
  savePosted: async function (account, postId) {
    ...
    db.findOne({ PostId: postId }, (err, post) => {
      if (!post)
        return console.log(
          `${account.name.toUpperCase()}: ${postId} was not found in the database.`
        );
      post.isPosted = true;
      post
        .save()
        .then(
          console.log(
            `${account.name.toUpperCase()}: ${postId} has been saved as posted to the database.`
          )
        )
        .catch((err) => console.log(err));
    });
